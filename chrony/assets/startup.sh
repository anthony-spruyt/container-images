...
# Drift file and PID file (in /var/lib/chrony to avoid /run permissions)
...
# Command socket for chronyc (in /var/lib/chrony to avoid /run permissions)
bindcmdaddress /var/lib/chrony/chrony.sock
...
# -U: Disable superuser check (allows running as non-root)
# -f: Configuration file
# -L: Log level (0=info, 1=warn, 2=error, 3=fatal)
exec /usr/sbin/chronyd -d -x -U -f "${CHRONY_CONF}" -L "${LOG_LEVEL}"
